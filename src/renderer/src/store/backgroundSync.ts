  ...
  try {
    ...
    if (newPls.length > 0) {
      ...
      for (const detail of created) {
        ...
        if (local) {
          setPlaylistCloudId(local.id, detail.id)
    ...
    // 4. Check for cloud-only playlists (created on other device)
    ...
    for (const summary of allSummaries) {
      ...
      // Already have it locally (maybe from a previous step)
      const localNameMatch = getPlaylists().some(
        (p) => p.name.toLowerCase().trim() === summary.title.toLowerCase().trim(),
      )
      if (localNameMatch) continue

      // New cloud-only playlist → create locally
