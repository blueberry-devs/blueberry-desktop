import { useEffect, useState } from 'react'
import { useTranslation } from '../utils/useTranslation'
import {
  useProfileState,
  fetchProfile,
  followProfile,
  unfollowProfile,
  closeProfile,
  isFollowingPending,
} from '../store/profiles'
import { useAuth } from '../store/auth'
import { getVerificationTier, getVerificationTooltip, decodeBadges, getBadges, type BadgeDef } from '../utils/badges'
import { getProfile } from '../store/profile'
import Tooltip from './Tooltip'
import './ProfileView.css'

function ProfileView(): JSX.Element {
  const { t } = useTranslation()
  const { viewing, cache } = useProfileState()
  const auth = useAuth()
  const [loading, setLoading] = useState(false)

  const profile = viewing ? cache[viewing] : null
  const isOwnProfile = auth.user && viewing === auth.user.username

  useEffect(() => {
    if (!viewing) return
    if (cache[viewing]) return

    setLoading(true)
    fetchProfile(viewing).finally(() => setLoading(false))
  }, [viewing])

  if (!viewing) {
    return (
      <div className="profile-view view-enter">
        <div className="profile-view__error">{t('profile.notFound')}</div>
      </div>
    )
  }

  const handleFollowToggle = (): void => {
    if (!profile) return
    if (profile.isFollowing) {
      unfollowProfile(viewing)
    } else {
      followProfile(viewing)
    }
  }

  const followingPending = isFollowingPending(viewing)
  const badgeIds = profile ? decodeBadges(profile.badgesMask) : []
  const badges = getBadges(badgeIds)

  return (
    <div className="profile-view view-enter">
      <button className="profile-view__back" onClick={closeProfile}>
        <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
          <path d="M10 3 5 8l5 5" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
        {t('common.back')}
      </button>

      {loading && !profile && (
        <div className="profile-view__loading">
          <div className="profile-view__spinner" />
        </div>
      )}

      {!profile && !loading && (
        <div className="profile-view__error">{t('profile.notFound')}</div>
      )}

      {profile && (
        <div className="profile-view__card">
          <div className="profile-view__avatar">
            {profile.avatarUrl ? (
              <img className="profile-view__avatar-img" src={profile.avatarUrl} alt="" />
            ) : (
              <div className="profile-view__avatar-placeholder">
                <svg width="32" height="32" viewBox="0 0 18 18" fill="none">
                  <path d="M14 15.5v-1a3 3 0 0 0-3-3H7a3 3 0 0 0-3 3v1" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" />
                  <circle cx="9" cy="6" r="3" stroke="currentColor" strokeWidth="1.2" />
                </svg>
              </div>
            )}
          </div>

          <div className="profile-view__username-row">
            <span className="profile-view__username">@{profile.username}</span>
            {profile.verificationLevel >= 1 && (
              <Tooltip text={getVerificationTooltip(profile.verificationLevel, getProfile().language)}>
                <span className="profile-view__verification-badge" style={{ color: getVerificationTier(profile.verificationLevel).color }}>
                  <svg width="16" height="16" viewBox="0 0 22 22" fill="currentColor">
                    <path d="M20.396 11c-.018-.646-.215-1.275-.57-1.816-.354-.54-.852-.972-1.438-1.246.223-.607.27-1.264.14-1.897-.131-.634-.437-1.218-.882-1.687-.47-.445-1.053-.75-1.687-.882-.633-.13-1.29-.083-1.897.14-.273-.587-.704-1.086-1.245-1.44S11.647 1.62 11 1.604c-.646.017-1.273.213-1.813.568s-.969.854-1.24 1.44c-.608-.223-1.267-.272-1.902-.14-.635.13-1.22.436-1.69.882-.445.47-.749 1.055-.878 1.688-.13.633-.08 1.29.144 1.896-.587.274-1.087.705-1.443 1.245-.356.54-.555 1.17-.574 1.817.02.647.218 1.276.574 1.817.356.54.856.972 1.443 1.245-.224.606-.274 1.263-.144 1.896.13.634.433 1.218.877 1.688.47.443 1.054.747 1.687.878.633.132 1.29.084 1.897-.136.274.586.705 1.084 1.246 1.439.54.354 1.17.551 1.816.569.647-.016 1.276-.213 1.817-.567s.972-.854 1.245-1.44c.604.239 1.266.296 1.903.164.636-.132 1.22-.447 1.68-.907.46-.46.776-1.044.908-1.681s.075-1.299-.165-1.903c.586-.274 1.084-.705 1.439-1.246.354-.54.551-1.17.569-1.816zM9.662 14.85l-3.429-3.428 1.293-1.302 2.072 2.072 4.4-4.794 1.347 1.246z" />
                  </svg>
                </span>
              </Tooltip>
            )}
          </div>

          <div className="profile-view__stats">
            <div className="profile-view__stat">
              <span className="profile-view__stat-value">{profile.followersCount}</span>
              <span className="profile-view__stat-label">{t('profile.followers')}</span>
            </div>
            <div className="profile-view__stat">
              <span className="profile-view__stat-value">{profile.followingCount}</span>
              <span className="profile-view__stat-label">{t('profile.following')}</span>
            </div>
          </div>

          {!isOwnProfile && auth.accessToken && (
            <button
              className={`profile-view__follow-btn${profile.isFollowing ? ' profile-view__follow-btn--following' : ' profile-view__follow-btn--follow'}${followingPending ? ' profile-view__follow-btn--loading' : ''}`}
              onClick={handleFollowToggle}
              disabled={followingPending}
            >
              {followingPending ? (
                <div className="profile-view__spinner" style={{ width: 14, height: 14 }} />
              ) : profile.isFollowing ? (
                <>
                  <span className="profile-view__follow-label">{t('profile.following')}</span>
                  <span className="profile-view__unfollow-label">{t('profile.unfollow')}</span>
                </>
              ) : (
                <>{t('profile.follow')}</>
              )}
            </button>
          )}

          {profile.badgesMask > 0 && badges.length > 0 && (
              <div className="profile-view__badges">
                {badges.map((b) => (
                  <div key={b.id} className="profile-view__badge" title={b.description}>
                    <span className="profile-view__badge-emoji">{b.emoji}</span>
                    {b.label}
                  </div>
                ))}
              </div>
            ) : null
          })()}
        </div>
      )}
    </div>
  )
}

export default ProfileView
